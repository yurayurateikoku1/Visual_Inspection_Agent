...
public:
    ...
    // ICameraCallback
    void onFrameReceived(const std::string &camera_id, const HalconCpp::HObject &frame) override;
...
private slots:
    ...
    void slot_cameraMaximizeRequested(const std::string &camera_id);

private:
    /// @brief 初始化相机
    void initCameras();
    /// @brief 动态添加新发现的相机 UI（view + 状态灯）
    void addCameraUI(const std::string &cam_id);
    ...
    void setCameraLed(const std::string &camera_id, bool online);
    /// @brief 设置通信状态灯
    void setCommLed(const std::string &comm_id, bool connected);
    /// @brief 设置光源状态灯
    void setLightLed(bool connected);

    Ui::MainWindow *ui;
    QGridLayout *camera_layout_ = nullptr;                   // 相机区域网格布局
    std::map<std::string, CameraViewWidget *> camera_views_; // 相机主件
    std::vector<CameraViewWidget *> camera_view_list_;       // 保持插入顺序
    std::string maximized_camera_id_;                        // 当前放大的相机
    std::map<std::string, QLabel *> camera_status_leds_;     // camera_id → 状态灯
    std::map<std::string, QLabel *> comm_status_leds_;       // comm_id → 状态灯
